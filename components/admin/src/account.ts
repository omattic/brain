...
export async function createAccountTenant(env: AccountEnv, token: string, input: {
  name: string;
  slug?: string;
  description?: string;
  service?: string;
}) {
  const payload = await accountRequest<{ tenant: AccountTenant }>(env, token, "/api/v1/tenants", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...input,
      service: input.service || "brain",
    }),
  });
  return payload.tenant;
}

export async function addAccountTenantMember(env: AccountEnv, token: string, tenantId: string, input: {
  email: string;
  role?: string;
  status?: string;
}) {
  const payload = await accountRequest<{ member: AccountTenantMember }>(
    env,
    token,
