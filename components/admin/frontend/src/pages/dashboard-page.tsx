...
type SectionId = "overview" | "tenants" | "runtime" | "monitoring";
...
const sections: Section[] = [
  {
    id: "overview",
    label: "Overview",
    caption: "Summary, session state, and operational posture.",
    icon: <Gauge className="h-4 w-4" />,
  },
  {
    id: "tenants",
    label: "Tenants",
    caption: "Provision tenants, memberships, and Meta account mappings.",
    icon: <Building2 className="h-4 w-4" />,
  },
  {
    id: "runtime",
    label: "Runtime Config",
    caption: "Write tenant-scoped settings that the runtime consumes.",
    ...
    >
      <div
        className={`mt-0.5 rounded-xl p-2 ${
          active ? "bg-[#635bff] text-white" : "bg-[#f4f5ff] text-[#635bff]"
        }`}
      >
        {icon}
      </div>
      <div className="min-w-0">
        <div className={`text-sm font-medium ${active ? "text-slate-950" : "text-slate-900"}`}>{label}</div>
...
export function DashboardPage() {
    ...
    return (
      <div className="space-y-6">
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Tenants" value={tenants.length} hint="Visible workspaces in scope." />
          <MetricCard label="Members" value={totalMembers} hint="Operators attached to visible tenants." />
          <MetricCard label="Meta Accounts" value={totalAccounts} hint="Mapped accounts feeding runtime routing." />
          <MetricCard label="Failed Events" value={events.length} hint="Queued failures awaiting triage." />
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.15fr,0.85fr]">
          <Card className="space-y-5">
            <SectionHeader
              ...
              description="This workspace runs on the shared Omattic auth session. Super-admin has global control; everyone else is tenant-scoped."
            />
          ...
          <Card className="space-y-5">
            <SectionHeader
              ...
              description="A quick operational picture of the currently loaded tenant scope."
            />
      ...
      <div className="space-y-6">
        <section className="grid gap-6 2xl:grid-cols-[0.92fr,1.08fr]">
          <Card className="space-y-5">
            <SectionHeader
              badge="Provisioning"
              title="Create and wire tenants"
              description="Provision tenants, seed users, and map Meta accounts without leaving the workspace."
            />
          ...
          <Card className="space-y-5">
            <SectionHeader
              ...
              description="Switch through the visible tenant footprint and inspect memberships, Meta accounts, and runtime attachments."
            />
      ...
      <div className="grid gap-6 2xl:grid-cols-[0.8fr,1.2fr]">
        ...
        <Card className="space-y-5">
          <SectionHeader
            ...
            description="A compact view of what is currently active in the visible tenant scope."
          />
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="text-3xl font-semibold tracking-tight text-slate-950">{totalConfigs}</div>
              <div className="mt-2 text-sm text-slate-500">Settings currently mirrored into runtime cache.</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-900">
              ...
              <div className="mt-2 text-sm text-slate-500">Operators with access in the visible scope.</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-900">
                <CreditCard className="h-4 w-4 text-[#635bff]" />
              ...
              <div className="text-3xl font-semibold tracking-tight text-slate-950">{totalAccounts}</div>
              <div className="mt-2 text-sm text-slate-500">Accounts mapped into tenant-aware routing.</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-6 2xl:grid-cols-[1.18fr,0.82fr]">
        <Card className="space-y-5">
          <SectionHeader
            ...
            description="Switch tenants quickly, inspect failures, and replay exactly what you need."
            actions={
              <Button
                variant="danger"
                className="gap-2"
        ...
        <Card className="space-y-5">
          <SectionHeader
            ...
            description="Inspect the stored event body before replaying or changing related configuration."
          />
  ...
  return (
    <Shell
      ...
      description="A left-nav workspace closer to a payment-ops dashboard: easier section switching, clearer focus, and a tighter mobile flow."
      actions={
        <>
    ...
    >
      ...
      <div className="grid gap-6 xl:grid-cols-[280px,minmax(0,1fr)]">
        ...
        <div className="space-y-6">
          ...
          {activeSection === "tenants" ? renderTenants() : null}
          {activeSection === "runtime" ? renderRuntime() : null}
