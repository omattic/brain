...
export async function verifyDashboardSession(request: Request) {
  ...
  const verifyUrl =
    process.env.DASHBOARD_AUTH_VERIFY_URL ||
    process.env.ADMIN_AUTH_VERIFY_URL ||
    "https://auth.omattic.com/verify";
  ...
  if (!payload?.authenticated || !payload?.user?.email) {
    return {
      ok: false as const,
      status: 401,
      error: "JWT is not authenticated for dashboard access",
    };
  }
